...
if __name__ == "__main__":
    ...
    # INIT_XYZS = np.array([[.3 * frameN, 0, .1] for frameN in range(ARGS.num_drones)])
    # INIT_RPYS = np.array([[0, 0,  frameN * (np.pi/3)/ARGS.num_drones] for frameN in range(ARGS.num_drones)])
    # NUM_WP = ARGS.control_freq_hz*15
    # TARGET_POS = np.zeros((NUM_WP,3))
    # for frameN in range(NUM_WP):
    #     if frameN < NUM_WP/6:
    #         TARGET_POS[frameN, :] = (frameN*6)/NUM_WP, 0, 0.5*(frameN*6)/NUM_WP
    #     elif frameN < 2 * NUM_WP/6:
    #         TARGET_POS[frameN, :] = 1 - ((frameN-NUM_WP/6)*6)/NUM_WP, 0, 0.5 - 0.5*((frameN-NUM_WP/6)*6)/NUM_WP
    #     elif frameN < 3 * NUM_WP/6:
    #         TARGET_POS[frameN, :] = 0, ((frameN-2*NUM_WP/6)*6)/NUM_WP, 0.5*((frameN-2*NUM_WP/6)*6)/NUM_WP
    #     elif frameN < 4 * NUM_WP/6:
    #         TARGET_POS[frameN, :] = 0, 1 - ((frameN-3*NUM_WP/6)*6)/NUM_WP, 0.5 - 0.5*((frameN-3*NUM_WP/6)*6)/NUM_WP
    #     elif frameN < 5 * NUM_WP/6:
    #         TARGET_POS[frameN, :] = ((frameN-4*NUM_WP/6)*6)/NUM_WP, ((frameN-4*NUM_WP/6)*6)/NUM_WP, 0.5*((frameN-4*NUM_WP/6)*6)/NUM_WP
    #     elif frameN < 6 * NUM_WP/6:
    #         TARGET_POS[frameN, :] = 1 - ((frameN-5*NUM_WP/6)*6)/NUM_WP, 1 - ((frameN-5*NUM_WP/6)*6)/NUM_WP, 0.5 - 0.5*((frameN-5*NUM_WP/6)*6)/NUM_WP
    # wp_counters = np.array([0 for frameN in range(ARGS.num_drones)])
