    ...
    def run(self):
        ...
        while self.episode_idx.value < self.glob_episode_thred:
            ...
            while not done:
                ...
                observation_, reward_def, _, done, info = self.env.step(action_def=action)
                score += reward_def
                self.local_actor_critic.remember(observation, action, reward_def)
...
def objective(trial):
    ...
    config = dict(glob_episode_thred=1300, gamma=0.39, lr=2.2e-6, LR_decay=0.89, pi_net_struc=[233, 285, 398, 287], v_net_struct=[233, 285, 398, 287])    # this config may be changed by optuna
    ...
    # ========= Save global model =========
    if on_server:
        path = "/home/zelin/Drone/code_files/data/defender"
    else:
        path = "/Users/wanzelin/办公/gym-drones/data/A3C/defender"
    os.makedirs(path + "/model", exist_ok=True)
    torch.save(global_actor_critic, path + "/model/trained_A3C_defender_" + str(start_time) + "-Trial_" + trial_num_str)
...
if __name__ == '__main__':
    ...
    if on_server:
        study = optuna.create_study(direction='maximize', study_name="A3C-hyperparameter-study",
                                    storage="sqlite://///home/zelin/Drone/code_files/data/defender/HyperPara_database.db",
                                    load_if_exists=True)
    else:
        study = optuna.create_study(direction='maximize', study_name="A3C-hyperparameter-study",
                                    storage="sqlite://///Users/wanzelin/办公/gym-drones/data/HyperPara_database.db",
                                    load_if_exists=True)
